...
void Enemy_fly::render(SDL_Renderer * renderer, int time_passed, CoreEngine & core)
{
	...
	else // its dead
	{
		if (direction < 0)
		{
			SDL_RenderCopy(renderer, core.enemy_fly_dead_texture, NULL, &pos_rect);

		}
		else
		{
			SDL_RenderCopyEx(renderer, core.enemy_fly_dead_texture, NULL, &pos_rect, 180.0f, NULL, SDL_FLIP_VERTICAL);
		}
	}
}
